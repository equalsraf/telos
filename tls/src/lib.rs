...
impl TlsContext {
    ...
    pub fn new_client() -> Option<TlsContext> {
        ...
        if p == ptr::null_mut() {
            ...
            Some(TlsContext { ptr: p, cfg: None})
        }
    }
    ...
    fn rv_to_result(&self, rv: i64) -> TlsResult<()> {
        if rv == 0 {
            ...
            Err(TlsError { msg: self.error(), code: rv })
        }
    }
    ...
    pub fn connect_servername(&mut self, hostname: &str, port: &str, servername: &str) -> TlsResult<()> {
        let rv = unsafe {
    ...
    fn rv_to_result_io(&self, rv: i64) -> TlsResult<usize> {
        match rv {
            ffi::WANT_POLLIN => Err(TlsError { msg: String::new(), code: rv}),
            ffi::WANT_POLLOUT => Err(TlsError { msg: String::new(), code: rv}),
            rv if rv < 0 => Err(TlsError { msg: self.error(), code: -1}),
            rv => Ok(rv as usize),
        }
    }
...
impl Drop for TlsConfig {
    fn drop(&mut self) {
        unsafe { ffi::tls_config_free(self.cfg); }
    }
}
...
impl TlsError {
    ...
    fn want_pollin(&self) -> bool { self.code == ffi::WANT_POLLIN }
    /// The operation failed because it would block writing
    fn want_pollout(&self) -> bool { self.code == ffi::WANT_POLLIN }
    fn wants_more(&self) -> bool { self.want_pollin() || self.want_pollout() }
}
